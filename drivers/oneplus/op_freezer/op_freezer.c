...
int op_freezer_report(enum message_type type, int caller_pid, int target_uid, const char *rpc_name, int code)
{
	...
	if (atomic_read(&op_freezer_deamon_port) == -1) {
		pr_debug("%s: op_freezer_deamon_port invalid!\n", __func__);
                return OP_FREEZER_ERROR;
	}

	if (sock_handle == NULL) {
		pr_debug("%s: sock_handle invalid!\n", __func__);
                return OP_FREEZER_ERROR;
	}

	if (type >= TYPE_MAX) {
		pr_debug("%s: type = %d invalid!\n", __func__, type);
		return OP_FREEZER_ERROR;
	}
	...
	if (skb == NULL) {
		pr_debug("%s: type =%d, nlmsg_new failed!\n", __func__, type);
		return OP_FREEZER_ERROR;
	}

	nlh = nlmsg_put(skb, 0, 0, 0, len, 0);
	if (nlh == NULL) {
		pr_debug("%s: type =%d, nlmsg_put failed!\n", __func__, type);
		kfree_skb(skb);
		return OP_FREEZER_ERROR;
	}
	...
	data = nlmsg_data(nlh);
	if(data == NULL) {
		pr_debug("%s: type =%d, nlmsg_data failed!\n", __func__, type);
		return OP_FREEZER_ERROR;
	}
	data->type = type;
